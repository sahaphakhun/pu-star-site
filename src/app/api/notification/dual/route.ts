...
import { sendBulkDualNotification } from '@/app/notification/dualNotification';
...
// API สำหรับส่งการแจ้งเตือนผ่านทั้ง SMS และ Messenger พร้อมกัน
export async function POST(req: Request) {
  try {
    ...
    const results = await sendBulkDualNotification(recipients, message);

    return NextResponse.json({ 
      success: true, 
      message: `ส่งสำเร็จ SMS: ${results.sms.success}/${recipients.length}, Messenger: ${results.messenger.success}/${recipients.length}`, 
      results: {
        sms: results.sms,
    ...
    console.error('เกิดข้อผิดพลาดในการส่งการแจ้งเตือนแบบคู่:', error);
    return NextResponse.json(
      { success: false, message: 'เกิดข้อผิดพลาดในการส่งการแจ้งเตือนแบบคู่' },
      { status: 500 }
    );
  }
} 