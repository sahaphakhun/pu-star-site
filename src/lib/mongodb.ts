...
// เพิ่ม fallback สำหรับ development และ build time
if (!MONGODB_URI) {
  if (process.env.NODE_ENV === 'development') {
    ...
    // ไม่ throw error ตอน build time เพราะจะทำให้ build fail
    // แทนที่จะ throw ให้ log warning และจะ throw ตอน runtime แทน
    console.warn('[DB] No MongoDB URI found in production environment - will fail at runtime if database is accessed');
  }
}
...
async function connectDB(): Promise<mongoose.Connection> {
  try {
    // ตรวจสอบ MONGODB_URI ตอน runtime
    if (!MONGODB_URI) {
      throw new Error(
