...
const ProductList: React.FC<ProductListProps> = ({
  ...
  const requiresSelection = (product: ProductWithId) => {
    const hasUnits = Array.isArray(product.units) && product.units.length > 0;
    const hasOptions = Array.isArray(product.options) && product.options.some(opt => Array.isArray(opt.values) && opt.values.length > 0);
    return hasUnits || hasOptions;
  };
            ...
            <motion.button
              ...
              onClick={() => {
                if (requiresSelection(product)) {
                  ...
                  // hint: อนาคตสามารถเปิดหน้ารายละเอียด/Modal เพื่อเลือกตัวเลือกได้
