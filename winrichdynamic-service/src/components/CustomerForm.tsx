...
interface CustomerFormData {
  ...
  priorityStar?: string; // 0-5 for UI select
  goals?: string;
  authorizedPhones?: string; // comma or newline separated
}
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData(prev => ({
        ...
        // map array fields to UI strings
        tags: Array.isArray((initialData as any).tags) ? (initialData as any).tags.join(', ') : '',
        priorityStar: (typeof (initialData as any).priorityStar === 'number' ? String((initialData as any).priorityStar) : '0'),
        goals: (initialData as any).goals || '',
        authorizedPhones: Array.isArray((initialData as any).authorizedPhones) ? (initialData as any).authorizedPhones.join(',') : '',
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData = {
        ...
        // transform UI-only fields
        tags: formData.tags
          ? formData.tags.split(/[\n,]/).map(s => s.trim()).filter(Boolean)
          : [],
        priorityStar: formData.priorityStar ? Number(formData.priorityStar) : 0,
        goals: formData.goals || undefined,
        authorizedPhones: formData.authorizedPhones
          ? formData.authorizedPhones.split(/[\n,]/).map(s => s.trim()).filter(Boolean).map((raw) => {
              // normalize to E.164 66xxxxxxxxx simple client-side helper
              let p = raw.replace(/[^0-9+]/g, '');
              if (p.startsWith('+')) p = p.substring(1);
              if (p.startsWith('0') && p.length === 10) {
                p = '66' + p.substring(1);
              } else if (p.startsWith('66') && p.length === 11) {
                // ok
              } else if (/^\d{9}$/.test(p)) {
                // if only 9 digits (already without 0), prefix 66
                p = '66' + p;
              }
              return p;
            })
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* แท็ก / ดาวความสำคัญ / เป้าหมาย */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              แท็ก (คั่นด้วย , หรือบรรทัดใหม่)
            </label>
            <textarea
              value={formData.tags}
              ...
              rows={2}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="เช่น: VIP, โรงงาน, กรุงเทพ"
            />
          </div>
          ...
          <div>
            ...
            <select
              value={formData.priorityStar}
              onChange={(e) => handleInputChange('priorityStar', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            เป้าหมาย/หมายเหตุสำคัญของลูกค้า
          </label>
          <textarea
            ...
            rows={3}
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            placeholder="สรุปสิ่งที่ลูกค้าคาดหวัง จุดสำคัญ ฯลฯ"
          />
        </div>

        {/* เบอร์ที่อนุญาตสำหรับเข้าสู่ระบบด้วยรหัสลูกค้า */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            เบอร์ที่อนุญาต (คั่นด้วย , หรือบรรทัดใหม่) รูปแบบ 66xxxxxxxxx
          </label>
          <textarea
            value={formData.authorizedPhones}
            onChange={(e) => handleInputChange('authorizedPhones', e.target.value)}
            rows={2}
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            placeholder="เช่น: 66812345678, 66987654321"
          />
          <p className="text-xs text-gray-500 mt-1">ระบบจะช่วยแปลงอัตโนมัติจาก 0xxxxxxxxx → 66xxxxxxxxx</p>
        </div>
        ...
        <div className="border-t pt-6">
          ...
          {showCompanyFields && (
            ...
            >
              ...
              {/* shipping moved out of company block */}
            </motion.div>
          )}
        </div>
